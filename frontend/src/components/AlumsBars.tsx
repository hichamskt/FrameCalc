...
import { useCreateAlumBar } from "../hooks/alumBar/useCreateAlumBar";
...
const ProductForm = ({
  ...
  isEdit: boolean;
  formData: AlumBar;
  setFormData: React.Dispatch<React.SetStateAction<AlumBar>>;
  uniqueProfileNames: string[];
  closeModals: () => void;
}) => (
...
function AlumsBars() {
  ...
  const { createAlumBar, newAlumBar } = useCreateAlumBar();
    ...
    if (alumBars) {
      const uniqueBars = alumBars.filter(
        (bar, index, self) =>
          index ===
          self.findIndex(
            (b) => b.profile_material_id === bar.profile_material_id
          )
      );
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesUnitType =
      unitTypeFilter === "all" || product.unit_type === unitTypeFilter;
    const matchesProfileName =
      profileNameFilter === "all" || product.profile_name === profileNameFilter;
  ...
  const confirmDelete = () => {
    if (selectedProduct) {
      setProducts(
        products.filter(
          (p) => p.profile_material_id !== selectedProduct.profile_material_id
        )
      );
  ...
  useEffect(()=>{
    if (newAlumBar) setProducts([...products, newAlumBar]);
    ...
    >
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-700/30">
                {filteredProducts.map((product, index) => (
                  <tr
                   key={product.profile_material_id}
                ...
                <p className="text-2xl font-bold text-white">
                  {products.length > 0
                    ? formatPrice(
                        (
                          products.reduce(
                            (sum, p) => sum + parseFloat(p.unit_price),
                            0
                          ) / products.length
                        ).toString()
                      )
      ...
      >
        <ProductForm 
          ...
          uniqueProfileNames={uniqueProfileNames}
          closeModals={closeModals}
        />
      </Modal>

      <Modal isOpen={showEditModal} onClose={closeModals} title="Edit Product">
        <ProductForm 
